...
import edu.sangerardo.edspace.proyecto.dto.CollectionDTO;
...
import edu.sangerardo.edspace.proyecto.facade.CollectionFacade;
...
public class CollectionService implements Serializable {
    ...
    public List<ViewCollectionDTO> getViewGroupByMonth() {
        List<ViewCollectionDTO> viewCollectionDTOs = new ArrayList<ViewCollectionDTO>();
        try {
            ...
            JSONArray data = solrService.getStatisticsResourceByType(ResourceTypeEnum.COLLECTION.getTipo(),startDate,endDate);
            ...
            for (int i = 0; i < data.length(); i++) {
                ...
                ViewCollectionDTO viewCollectionDTO = getViewCollectionDTO(viewCollectionDTOs, id);
                if (viewCollectionDTO == null) {
                    viewCollectionDTO = new ViewCollectionDTO();
                    viewCollectionDTO.setId(id);
                    viewCollectionDTO.setIp(!object.isNull(SolrEnum.IP.getTipo()) ? object.getString(SolrEnum.IP.getTipo()) : "");
                    viewCollectionDTO.setEpersonId(!object.isNull(SolrEnum.EPERSON_ID.getTipo()) ? object.getInt(SolrEnum.EPERSON_ID.getTipo()) : 0);
                    CollectionDTO collectionDTO = new CollectionDTO();
                    collectionDTO.setCollection(collectionFacade.find(viewCollectionDTO.getId()));
                    if (collectionDTO.getCollection() == null) {
                        continue;
                    }
                    List<Metadatavalue> metadatavalues = metaDataValueFacade.findByNamedQueryWithLimit("Metadatavalue.findByResource", 0, collectionDTO.getCollection().getCollectionId(),
                            Integer.parseInt(ResourceTypeEnum.COLLECTION.getTipo()), Integer.parseInt(MetaDataEnum.TITLE.getTipo()));
                    collectionDTO.setTitle(!metadatavalues.isEmpty() ? metadatavalues.get(0).getTextValue() : "");
                    viewCollectionDTO.setCollectionDTO(collectionDTO);
                    viewCollectionDTO.setTime(time);
                    viewCollectionDTOs.add(viewCollectionDTO);
                }
                ...
                for (String dateRangeString : datesRangeString) {
                    ResourceSolrGroupByDTO resourceSolrGroupByDTO = getViewCollectionGroupBy(viewCollectionDTO.getResourceStatisticGroupBys(), dateRangeString);
                    if (resourceSolrGroupByDTO == null) {
                        ...
                        viewCollectionDTO.getResourceStatisticGroupBys().add(resourceSolrGroupByDTO);
                    } else {
                        if (dateRangeString.equals(key)) {
                            resourceSolrGroupByDTO.setNumberOperation(resourceSolrGroupByDTO.getNumberOperation() + 1);
                        }
        ...
        return viewCollectionDTOs;
    }

    private ViewCollectionDTO getViewCollectionDTO(List<ViewCollectionDTO> viewCollectionDTOs, Integer id) {
        for (ViewCollectionDTO viewCollectionDTO : viewCollectionDTOs) {
            if (id.equals(viewCollectionDTO.getId())) {
                return viewCollectionDTO;
            }
        }
        return null;
    }

    private ResourceSolrGroupByDTO getViewCollectionGroupBy(List<ResourceSolrGroupByDTO> resourceStatisticGroupByDtos, String date) {
